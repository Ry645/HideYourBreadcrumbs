...
var exposureRecoverTime:float = 1.5 #in seconds
...
func _on_body_entered(body):
	if body is Player:
		body.camera.interpolateToExposure(alteredCameraExposure, exposureDarkenTime)


func _on_body_exited(body):
	if body is Player:
		body.camera.interpolateToExposure(defaultCameraExposure, exposureRecoverTime)
